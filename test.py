...
sys.path.insert(1, os.path.join(curr_dir, "src", "src"))
sys.path.insert(1, os.path.join(curr_dir, "src", "src", "java"))
sys.path.insert(1, os.path.join(curr_dir, "src", "src", "jbse"))
...
symmgr = {
    ("V", 0): JBSESymbolValue(0, JavaTypeFloat()),
    ("V", 1): JBSESymbolValue(1, JavaTypeChar()),
    ("V", 3): JBSESymbolValue(3, JavaTypeFloat()),
}
...
# string = "((WIDEN-ID(NARROW-FI((0.3f) + ({V3})) + NARROW-FI(3.0f))) + (WIDEN-ID(NARROW-FI((0.3f) + ({V3})) + WIDEN-CI(40))))"
string = "(WIDEN-ID((NARROW-FI((0.3f) + ({V0}))) + (3))) + (WIDEN-ID((NARROW-FI((0.3f) + ({V0}))) + (WIDEN-CI({V1})) + (40))) >= (0.0d)"

string = replace_conv(string)

# number literals
num_lit_pattern = r"(\(([+-]?)(\d[\d\.]*)([dfL]?)\))"
string = re.sub(num_lit_pattern, lambda match: replace_num_lit(*match.groups()), string)

# value symbols
val_sym_pattern = r"(\{V(\d+)\})"
string = re.sub(val_sym_pattern, lambda match: replace_val_sym(*match.groups()), string)
